...
{   float movLooper = 0.15f;
	...
	public List <Texture> Textureados = new List<Texture> ();
	Transform marker
	{
		get
		{
	...
	void TextureTile(Point p){
		
	//	if (tiles.ContainsKey (p))
	//		return;
		Tile t = tiles [p];
		Texture tex = Textureados [1];
		t.SetTileTexture (tex);
		
		//add a loop to switch between textures?
	}
